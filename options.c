...
void mm_mapopt_init(mm_mapopt_t *opt)
{
	...
	opt->pe_ori = 0; // FF
	opt->pe_bonus = 33;
	opt->dupmap = 1;             //dupmap default: 0, do not perform dupmap
	opt->dupmap_topn = 5;        //dupmap first top n chains
	opt->dupmap_dupm = 10;       //dupmap then top m chains with high shared length with the first chain
	opt->dupmap_dup_length = 50; //dupmap min dup length
	opt->kmer_length = 15;       //dupmap kmer length
