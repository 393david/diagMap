...
typedef struct { 
	uint64_t pos_ref_start;
	uint64_t pos_ref_end;
	uint64_t pos_query_start;
	uint64_t pos_query_end;
	int64_t anchor_index_start; //start anchor index of this chain
	int64_t anchor_index_end;   // end anchor index of this chain
} mm128_t_each_chain;//mymymy store anchor positions for each chains by using 45 scoring
typedef struct { 
	int32_t pos_ref_start;
	int32_t pos_ref_end;
	int32_t pos_query_start;
	int32_t pos_query_end;
	int64_t anchor_index_start; //start anchor index of this chain
	int64_t anchor_index_end;   // end anchor index of this chain
	int32_t anchor_num;         // record the number of anchors from start to end
	float   anchor_diversity;   // anchor diversity = anchor_num / (pos_ref_end - pos_ref_start)
								// is used to filter low anchor_diversity
	int8_t  plus;               // plus or minus direction
	int8_t  rechain;            // whether rechain or not, 0: initialized as 0. 1: for rechain
} mm64_t_each_chain;//mymymy store anchor positions for each chains by using 45 scoring
...
typedef struct {
	...
	int dupmap;   //dupmap perform dupMap or not
	int dupmap_topn;        //dupmap first top n chains
	int dupmap_dupm;        //dupmap then top m chains with high shared length with the first chain
	int dupmap_dup_length;  //dupmap min dup length
	int kmer_length;  //dupmap min dup length
} mm_mapopt_t;
