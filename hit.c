		...
		for (j = 0; j < k; ++j) { // traverse existing primary hits again
			...
			if ((float)ol / min - (float)uncov_len / max > mask_level && uncov_len <= mask_len) { // then this is a secondary hit
				...
				if (rp->p && ri->p && (rp->rid != ri->rid || rp->rs != ri->rs || rp->re != ri->re || ol != min)) { // the last condition excludes identical hits after DP
					...
					if (rp->p->dp_max - ri->p->dp_max <= sub_diff) {
						cnt_sub = 1;
					}
				}
				if (cnt_sub) {
...
int compare_uint64_decrease(const void *a, const void *b) {
    uint64_t num1 = *(const uint64_t*)a;
    uint64_t num2 = *(const uint64_t*)b;
    return (num1 < num2) - (num1 > num2);
}
int compare_int32_increase(const void *a, const void *b) {
    int32_t x = *(const int32_t*)a;
    int32_t y = *(const int32_t*)b;
    return (x < y) ? -1 : ((x > y) ? 1 : 0);
}
//dupmap select the top N + M chains
void mm_select_sub_dupmap(void *km, const mm_mapopt_t *opt, int min_diff, int best_n, int check_strand, int min_strand_sc, int *n_, mm_reg1_t *r, mm128_t *a, int qlen)
{
	if (0){
	//if (*n_ > opt->dupmap_topn + opt->dupmap_dupm){
		int n = *n_, j, k;//, , n_dup = 0, remain_chains_num = n - opt->dupmap_topn, shared_len_now, shared_num_now;// last_index;
		//uint64_t *shared_length = NULL, *shared_num = NULL;
		int32_t boundary_left, boundary_right;
		boundary_left  = (int32_t)a[r[0].as].x >= (int32_t)a[r[0].as].y * 1.2 ? (int32_t)a[r[0].as].x - (int32_t)a[r[0].as].y * 1.2: 0;
		boundary_right = (int32_t)a[r[0].as + r[0].cnt - 1].x + (qlen - (int32_t)a[r[0].as + r[0].cnt - 1].y) * 1.2;
		//int32_t pos_in_query_j_start, pos_in_query_j_end;
		k = opt->dupmap_topn;
		for (j = opt->dupmap_topn; j < *n_; j++){
			//pos_in_query_j_start = (int32_t)a[r[j].as].y;
			//pos_in_query_j_end   = (int32_t)a[r[j].as + r[j].cnt - 1].y;
			if (r[j].rid == r[0].rid){
				if(r[j].rs >= boundary_left && r[j].re <= boundary_right){
					r[k++] = r[j];
					if (k >= opt->dupmap_topn + opt->dupmap_dupm){
						break;
					}
				}
			}
...
/*		
		...
		int32_t *arr32 = NULL;
		int32_t pos_in_query_i, pos_in_query_j, pos_index_j, pos_index_i, min_close = INT32_MAX, min_indexx = 0;
		int32_t pre_index_chain_i = -1, pre_index_chain_j = -1, chain_len, bili_int, indexx, kk = 0;
		int32_t pos_in_query_j_start, pos_in_query_j_end;
		float bili;
	    //shared_length = (uint64_t*)kmalloc(km, remain_chains_num);
		shared_length = (uint64_t*)malloc(remain_chains_num * sizeof(uint64_t));
		shared_num    = (uint64_t*)malloc(remain_chains_num * sizeof(uint64_t));
		arr32 = (int32_t*)malloc(opt->dupmap_dupm * sizeof(int32_t));
		for (j = opt->dupmap_topn; j < *n_; j++){
			shared_len_now = 0;
			shared_num_now = 0;
			pos_in_query_j_start = (int32_t)a[r[j].as].y;
			pos_in_query_j_end   = (int32_t)a[r[j].as + r[j].cnt - 1].y;
			if (pos_in_query_j_start < (int32_t)a[r[0].as + r[0].cnt - 1].y && pos_in_query_j_start > (int32_t)a[r[0].as].y){
				for(k = 0; k < r[j].cnt; k++){
					pos_index_j = k + r[j].as;
					pos_in_query_j = (int32_t)a[pos_index_j].y;
					for (i = min_indexx; i < r[0].cnt; i++){
						pos_index_i = i + r[0].as;
						pos_in_query_i = (int32_t)a[pos_index_i].y;
						if(abs(pos_in_query_i - pos_in_query_j) < min_close){
							min_close = abs(pos_in_query_i - pos_in_query_j);
							min_indexx = i;
						}
						else{
							if(min_close == 0){
								shared_num_now += 1;
								if(pre_index_chain_i == -1 && pre_index_chain_j == -1){
									shared_len_now += opt->kmer_length;
									pre_index_chain_i = min_indexx + r[0].as;
									pre_index_chain_j = pos_index_j;
								}
								else if (((int32_t)a[min_indexx].y - (int32_t)a[pre_index_chain_i].y == (int32_t)a[pos_index_j].y - (int32_t)a[pre_index_chain_j].y) && (int32_t)a[pos_index_j].y - (int32_t)a[pre_index_chain_j].y < opt->kmer_length){
									shared_len_now += (int32_t)a[min_indexx].y - (int32_t)a[pre_index_chain_i].y;
									pre_index_chain_i = min_indexx + r[0].as;
									pre_index_chain_j = pos_index_j;
								}
								else if((int32_t)a[min_indexx].y - (int32_t)a[pre_index_chain_i].y > opt->kmer_length && (int32_t)a[pos_index_j].y - (int32_t)a[pre_index_chain_j].y > opt->kmer_length){
									shared_len_now += opt->kmer_length;
									pre_index_chain_i = min_indexx + r[0].as;
									pre_index_chain_j = pos_index_j;
								}
				...
				chain_len = r[j].re - r[j].rs;
			}
			else{
				chain_len = 1;
			}				
			bili = shared_len_now * 1.0 / chain_len;
			bili_int = (int)(bili * 10000000.0);
			shared_length[kk] = (uint64_t)bili_int << 32 | j;
			shared_num[kk++]    = (uint64_t)shared_num_now << 32 | j;
			min_indexx = 0;
			pre_index_chain_i = -1;
			pre_index_chain_j = -1;
			//shared_len_now = 0;
		}
		//radix_sort_64(shared_length, shared_length + remain_chains_num);
		qsort(shared_length, remain_chains_num, sizeof(uint64_t), compare_uint64_decrease);
		qsort(shared_num, remain_chains_num, sizeof(uint64_t), compare_uint64_decrease);
		// find top opt->dupmap_dupm index with longest shared length
		for (i = 0; i < opt->dupmap_dupm; i++) {
        	arr32[i] = (int32_t)shared_length[i];
    	}
		qsort(arr32, opt->dupmap_dupm, sizeof(int32_t), compare_int32_increase);
		kk = 0;
		for (i = k = opt->dupmap_topn; i < opt->dupmap_topn + opt->dupmap_dupm; i++){
			indexx = arr32[kk];
			r[k++] = r[indexx];
			kk++;
		}
		//for(i = opt->dupmap_topn + opt->dupmap_dupm; i < *n_; i++){
		//	indexx = (int32_t)shared_length[remain_chains_num - kk];
		//	if (r[indexx].p){
		//		free(r[i].p);
		//	}
		//}
		//for(i = opt->dupmap_topn; i < *n_; i++){
		//	//indexx = (int32_t)shared_length[remain_chains_num - kk];
		//	if (r[i].p){
		//		free(r[i].p);
		//	}
