...
int compare_y(const void* a, const void* b) {
    return (int)(((mm128_t*)a)->y - ((mm128_t*)b)->y);
}
								  ...
								  int *n_mini_pos, uint64_t **mini_pos)
{
	...
	//mymy sort position in query
	int64_t start = 0, n = *n_a;
	for (i = 1; i <= n; i++) {
		if (i == n || (int32_t)a[i].x != (int32_t)a[start].x){
			qsort(&a[start], i - start, sizeof(mm128_t), compare_y);
			start = i;
		} 
	}
	return a;
...
{
	//dupmap if (!(opt->flag & MM_F_ALL_CHAINS)) { // don't choose primary mapping(s)
	...
	//}
	//else if(opt->dupmap){
	mm_select_sub_dupmap(km, opt, mi->k*2, opt->best_n, 1, opt->max_gap * 0.8, n_regs, regs, a, qlen);
	//}
}
...
static mm_reg1_t *align_regs(const mm_mapopt_t *opt, const mm_idx_t *mi, void *km, int qlen, const char *seq, int *n_regs, mm_reg1_t *regs, mm128_t *a)
{
	...
	//dupmap if (!(opt->flag & MM_F_ALL_CHAINS)) { // don't choose primary mapping(s)
	//d	mm_set_parent(km, opt->mask_level, opt->mask_len, *n_regs, regs, opt->a * 2 + opt->b, opt->flag&MM_F_HARD_MLEVEL, opt->alt_drop);
	//d	mm_select_sub(km, opt->pri_ratio, mi->k*2, opt->best_n, 0, opt->max_gap * 0.8, n_regs, regs);
...
void mm_map_frag_core(const mm_idx_t *mi, int n_segs, const int *qlens, const char **seqs, int *n_regs, mm_reg1_t **regs, mm_tbuf_t *b, const mm_mapopt_t *opt, const char *qname)
{
	...
	if (opt->flag & MM_F_RMQ) {
		...
		a = mg_lchain_dp_first_locate(max_chain_gap_ref, max_chain_gap_qry, opt->bw, opt->max_chain_skip, opt->max_chain_iter, opt->min_cnt, opt->min_chain_score,
						 chn_pen_gap, chn_pen_skip, is_splice, n_segs, &n_a, a, &n_regs0, &u, b->km, qlens);
	}
	//fprintf(stdout, "q_name\tchr_name\tpos_ref\tpos_query\tstrand\n");
	//for (j = 0; j < n_a; ++j){		
	//	fprintf(stdout, "%s\t%s\t%d\t%d\t%c\n", qname, mi->seq[a[i].x<<1>>33].name,(int32_t)a[j].x, (int32_t)a[j].y, "+-"[a[j].x>>63]);
	//}
	//kfree(b->km, a);
	//return;	
	a = mg_lchain_dp_45_diagonal(max_chain_gap_ref, max_chain_gap_qry, opt->bw, opt->max_chain_skip, opt->max_chain_iter, opt->min_cnt, opt->min_chain_score,
						 chn_pen_gap, chn_pen_skip, is_splice, n_segs, n_a, a, &n_regs0, &u, b->km, qlens);
	//kfree(b->km, a);
	//return;
	...
	if (b->km) {
		...
		//fprintf(stderr, "DEBUG: %s\t%d, kmst.n_blocks = %d, kmst.n_cores = %d\n", qname, qlen_sum, kmst.n_blocks, kmst.n_cores);
		//fflush(stdout);
		if (kmst.n_blocks != kmst.n_cores) {
    		//fprintf(stderr, "[FIX] %s: n_blocks(%d) → n_cores(%d)\n", qname, kmst.n_blocks, kmst.n_cores);
    		kmst.n_blocks = kmst.n_cores; // 同步块数到核心数
		}
